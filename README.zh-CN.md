...
- 先选中拓扑节点或临时点，再拖动其 X/Y/Z 三轴控件来更新位置，避免直接拖点造成误移动。
- 按住 `Ctrl`/`Cmd` 可选中两个节点，然后在 3D 视图中右键选择 `Add Edge` 创建边。
...
- 添加或删除拓扑边；选中 edge 后，在输入框未聚焦时也可按 `Delete` 或 `Backspace` 删除。
- 修改节点 `type`。
- 默认节点类型：
  - `junction`
  - `charge`
...
5. 在 3D 场景中先选中节点或临时 topo point，再拖动 X/Y/Z 箭头移动点位；平面拖动已关闭。
6. 按住 `Ctrl`/`Cmd` 选中两个节点，右键选择 `Add Edge` 创建边。
7. 在选中节点面板中编辑节点坐标、类型、ID 或 `Rotation Z`。
8. 需要调整节点编号时，在 `Nodes` 列表中拖拽节点顺序。
9. 选中 edge 后，可编辑路径点、临时 topo point、锁定状态和单个路径点朝向。
10. 调整 spacing 重新生成插值路径。
11. 如果加载后的路线方向相反，可在 `Path Generation` 中点击反转方向按钮。
12. 需要回退时使用 `Undo` 或 `History`。
13. 需要时在 `Appearance` 中修改背景颜色、点云大小、点云颜色或切换 6 面视角。
14. 导出编辑后的拓扑 JSON。
